        ...
        <section className="space-y-4">
          <div>
            <h2 className="flex gap-1 text-2xl items-end">
              <p>{ today }</p>
              <p className="text-lg">({ format(firstDay, "dd.MM.yyyy") })</p>
            </h2>
            <Container className="gap-10 px-6 items-center"> 
              
              {/* температура */}
              <div className="flex flex-col px-4 items-center">
                <span className="text-5xl">{Math.round(data.list[0].main.temp)??'н/о'}°</span>
                <p className="text-xs space-x-1 whitespace-nowrap">
                  <span>Ощущается как</span>
                  <span>{Math.round(data.list[0].main.feels_like)??'н/о'}°</span>
                </p>
                <p className="text-xs space-x-2">
                  <span>{Math.round(data.list[0].main.temp_min)??'н/о'}°↓</span>
                  <span>{Math.round(data.list[0].main.temp_max)??'н/о'}°↑</span>
                </p>
              </div>
              ...
              <div className="flex gap-10 sm:gap-16 overflow-x-auto w-full justify-between pr-3">
                {data.list.map((item, idx) => (
                  <div key={idx} className="flex flex-col justify-between gap-2 items-center text-xs font-semibold">
                    ...
                    <p>{Math.round(item.main.temp)??'н/о'}°</p>
                  </div>
                ))}
              </div>
          ...
          <div className="flex gap-4">
            ...
            <Container className="bg-yellow-300/80 px-6 gap-4 justify-between overflow-x-auto">
              <WeatherDetail
                ...
                windSpeed={`${Math.round(weather.wind.speed)} м/с`}
